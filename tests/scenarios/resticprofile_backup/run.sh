#!/usr/bin/env bash
# Run the Docker-backed Resticprofile role scenario: converge twice, exercise a
# real temporary repository and canary restore, then verify explicit teardown.
# It creates a disposable local container and test artifacts, never production
# inventory or repositories.
set -euo pipefail

scenario_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_root="$(cd "${scenario_dir}/../../.." && pwd)"
work_dir="${repo_root}/.ansible-tmp/scenarios/resticprofile_backup"
image_name="homeops-ansible-ubuntu-ssh-target:22.04"
container_name="homeops-resticprofile-backup-$$"
...
printf '%s\n' '[resticprofile_backup_targets]' > "${inventory_file}"
printf '%s%s%s%s%s\n' \
  'resticprofile-target ansible_host=127.0.0.1' \
...
echo "=== converge: resticprofile_backup ==="
ansible-playbook -i "${inventory_file}" "${scenario_dir}/converge.yml"

echo "=== idempotence: resticprofile_backup ==="
...
echo "=== verify: resticprofile_backup ==="
ansible-playbook -i "${inventory_file}" "${scenario_dir}/verify.yml"

echo "=== teardown: resticprofile_backup ==="
