...
work_dir="${repo_root}/.ansible-tmp/scenarios/ssh_users"
image_name="homeops-ansible-ubuntu-ssh-target:22.04"
container_name="homeops-ssh-users-$$"
...
# Pre-create groups so the scenario can verify explicit removal and that
# unrelated groups are left alone.
docker exec "${container_name}" groupadd homeops_absent_group
docker exec "${container_name}" groupadd homeops_unrelated_group
...
[ssh_users_targets]
ssh-users-target ansible_host=127.0.0.1 ansible_port=${ssh_port} ansible_user=ansibletest ansible_ssh_private_key_file=${controller_key} ansible_python_interpreter=/usr/bin/python3
EOF
...
# Verify asserts group, user, key generation/export, authorized_keys, and
# hardening state.
echo "=== verify: ssh_users ==="
ansible-playbook -i "${inventory_file}" "${scenario_dir}/verify.yml"

echo
echo "=== manual inspection: ssh_users ==="
echo "Expected remote state:"
echo "  - certificates group exists"
echo "  - homeops_absent_group is absent"
echo "  - homeops_unrelated_group still exists"
echo "  - homeops_test_user exists and is in certificates"
echo "  - /home/homeops_test_user/.ssh/id_homeops_full_ed25519 exists with mode 0600"
echo "  - generated public key is exported under ${artifacts_dir}/ssh_keys"
echo "  - authorized_keys contains inline, file, and generated public keys"
echo "  - sshd effective config disables password and root login"
echo
echo "Remote groups:"
docker exec "${container_name}" getent group certificates homeops_unrelated_group
echo
echo "Managed user groups:"
docker exec "${container_name}" id homeops_test_user
echo
echo "Remote authorized_keys:"
docker exec "${container_name}" cat /home/homeops_test_user/.ssh/authorized_keys
echo
echo "Remote generated public key:"
docker exec "${container_name}" cat /home/homeops_test_user/.ssh/id_homeops_full_ed25519.pub
echo
echo "Controller exported generated public key:"
cat "${artifacts_dir}/ssh_keys/homeops_test_user@ssh-users-target.pub"
